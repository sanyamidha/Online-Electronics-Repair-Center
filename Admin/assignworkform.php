 ...
 if(isset($_REQUEST['assign'])){
  ...
  if(($_REQUEST['request_id'] == "") || ($_REQUEST['request_info'] == "") || ($_REQUEST['requestdesc'] == "") || ($_REQUEST['requestername'] == "") || ($_REQUEST['address1'] == "") || ($_REQUEST['address2'] == "") || ($_REQUEST['requestercity'] == "") || ($_REQUEST['requesterstate'] == "") || ($_REQUEST['requesterzip'] == "") || ($_REQUEST['requesteremail'] == "") || ($_REQUEST['requestermobile'] == "") || ($_REQUEST['assigntech'] == "") || ($_REQUEST['inputdate'] == "")){
   ...
   $msg = '<div class="alert alert-warning col-sm-6 ml-5 mt-2" role="alert"> Fill All Fileds </div>';
    ...
    $sql = "INSERT INTO assignwork_tb (request_id, request_info, request_desc, requester_name, requester_add1, requester_add2, requester_city, requester_state, requester_zip, requester_email, requester_mobile, assign_tech, assign_date) VALUES ('$rid', '".addslashes($rinfo)."','".addslashes($rdesc)."', '".addslashes($rname)."', '".addslashes($radd1)."', '".addslashes($radd2)."', '".addslashes($rcity)."', '".addslashes($rstate)."', '$rzip', '$remail', '$rmobile', '".addslashes($rassigntech)."', '$rdate')";
  ...
  <form action="" method="POST">
    ...
    <div class="form-row">
      <div class="form-group col-md-4">
        ...
        <input type="text" class="form-control" id="requestercity" name="requestercity" value="<?php if(isset($row['requester_district'])) {echo $row['requester_district']; }?>">
      </div>
      <div class="form-group col-md-4">
        <label for="requesterstate">Division</label>
        <input type="text" class="form-control" id="requesterstate" name="requesterstate" value="<?php if(isset($row['requester_div'])) { echo $row['requester_div']; } ?>">
      </div>
      <div class="form-group col-md-4">
